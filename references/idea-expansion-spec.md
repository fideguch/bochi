...
- **長い transcript (> 3 分動画) は直接読まず必ずサブエージェントに要約依頼** — `realtime-access-methods.md` の "Sub-agent summarisation pattern" に従う (pokemon-champions skill から取り込んだ pattern)
...
`~/bochi-data/topics/YYYY-MM-DD-{slug}.md` に自動保存。
...
After writing the topic file, append to index.jsonl via Read→append→Write pattern:
1. Read `~/bochi-data/index.jsonl`
2. Add new JSONL line: `{"id":"topic-YYYYMMDD-NNN","type":"topic","title":"...","date":"...","category":"...","tags":[...],"freshness":"active","channel":"cli","path":"topics/YYYY-MM-DD-slug.md"}`
3. Write tool で書き出し

Verified sources → append to `sources/verified.jsonl` (same Read→append→Write pattern):
`{"url":"...","domain":"...","eeat_score":32,"date":"...","topic_id":"..."}`
