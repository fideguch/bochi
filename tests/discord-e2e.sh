...
if [ "$BOT_MSG_COUNT" -gt 0 ]; then
  ...
  if [ "$TOTAL_YU_ALL" -gt 0 ]; then
    ...
    # v2.6: DM responder is the Mac Claude bridge (Claude voice, no yu persona).
    # Newspaper content generated by bochi may still use yu. Informational only.
    RATIO=$((TOTAL_YU_HIT * 100 / TOTAL_YU_ALL))
    warn "CH-01 (informational): yu suffix ratio ${RATIO}% — persona check retired in v2.6 (Mac bridge speaks as Claude)"
  else
