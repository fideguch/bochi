...
## v2.7.4 (2026-07-30) — Newspaper Event Coupling（時刻結合をやめて取りこぼしを止める）

### Why

7/27・7/28・7/30 の朝刊が Discord に届かなかった。v2.7.3 で「Known / Not Fixed」に挙げた
新聞パイプラインの積み残しで、**根はブリッジ障害と同じホストのスリープ**。

### Root Cause（7/30 のログで現行犯を確定）

```
2026-07-29T21:27:52Z generate start (2026-07-30)
2026-07-29T23:30:44Z deliver → SKIP: no newspaper for 2026-07-30   ← 配信が先に走った
2026-07-29T23:39:15Z generate ok (exit=0)                          ← 8分35秒 遅かった
```

1. **時刻結合の破綻**: 「06:20 生成 → 08:00 配信」は生成が 1時間40分以内に終わる前提だった。
   スリープで凍結されると成立しない（7/30 の生成は実時間 **2時間11分**）。配信ジョブは
   **8分35秒** 早く走って空振りし、`2026-07-30.md` はディスク上に存在するのに未配信で放置された。
2. **launchd の定時ジョブがスリープで落ちる**: `StartCalendarInterval` の発火は
   gen / deliver とも **21日で13回（62%）**。両ジョブが独立に遅延するため相対順序が保証されない。
3. **生成失敗が無言**: 7/26 は `claude -p exit=1` で失敗したが `notify-owner` が呼ばれず、
   2日間誰も気づかなかった。

### Fixed

- **イベント結合に変更**: `generate-newspaper.sh` が publish 成功直後に
  `deliver-newspaper.sh` を**その場で呼ぶ**（`deliver_now()`）。時計に依存しない。
  08:00 の定時ジョブはバックストップとして残す。
- **配信の冪等化**: `deliver-newspaper.sh` に日次マーカー
  `bochi-data/newspaper/.<DATE>.delivered` を導入。インライン呼び出しとバックストップの
  二重送信を防ぐ。`--force` で意図的な再送は可能。マーカーは30日で自動削除。
- **マーカーは「実送信」時のみ書く**: python は「新聞なし」「不正形式」のスキップ時も
  exit 0 を返すため、終了コードで判定すると未送信を配信済みとマークしてバックストップを
  永久に殺す。標準出力の `--- run ok ---`（最後のメッセージ送出後にのみ出る）で判定する。
  `--dry-run` はこの行を出さないので誤マークしない（実測確認）。
- **既存分の救済**: 生成済み・未配信の号は、次回 generate 実行時（`already generated` 経路）
  にも配信を試みる。マーカーがあれば no-op。
- **生成失敗を通知**: 形式不正 / 生成物なしの両経路で `notify-owner.sh` を呼ぶ。

### Verified

- `bash -n` / `shellcheck -S warning` ともに 2ファイルクリーン。macOS の bash 3.2 で
  空配列展開 `${ARGS+"${ARGS[@]}"}` が引数ゼロになることを実機確認。
- 7/30 の未配信号を手動配信し到達確認（`run ok`・6/6 送信）。
- マーカーあり → `deliver` 単体で **SKIP**、`generate` の `already generated` 経路からの
  インライン呼び出しも **SKIP**（Discord への重複送信ゼロ）。
- 未検証: 「新規生成 → その場で送信」の一連は明朝 06:20 JST が初回。送信自体は 7/30 の
  手動配信で、呼び出し配線は上記テストで別々に確認済み。
