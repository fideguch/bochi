...
1. `/home/ubuntu/bochi-data/` の存在確認（index.jsonl, user-profile.yaml）
...
- `cache/calendar.md` → 直近3日の予定（Mac launchd毎時更新→S3同期）
- `cache/gmail.md` → 直近メール（Mac launchd毎時更新→S3同期）
...
| Path | Purpose |
| ------ | --------- |
| /home/ubuntu/bochi-data/ | All persistent data (index, topics, memos, cache) |
| ~/.claude/skills/bochi/ | Skill definition + reference specs |
| ~/.claude/skills/bochi/references/ | On-demand spec files (load per mode) |

### Write Method (HARD-GATE)

<HARD-GATE>
bochi-data への書き込みパスは **必ず `/home/ubuntu/bochi-data/`** を使用する。
`~/.claude/bochi-data/` や `~/.claude/` を含むパスを file_path に指定してはならない。
Claude Code は .claude/ ディレクトリへの書き込みをハードコードでブロックする。
--dangerously-skip-permissions、hooks、settings のいずれも .claude/ ブロックを回避できない。

書き込みは Write/Edit ツールを使用する。Bash の `echo >>` や `cat >` は使わない。
- index.jsonl への追記: Read → 末尾に新行追加 → Write `/home/ubuntu/bochi-data/index.jsonl`
- memos/ への新規作成: Write `/home/ubuntu/bochi-data/memos/...`
- cache/ への書き込み: Write `/home/ubuntu/bochi-data/cache/...`
- seen.jsonl への追記: Read → 末尾に新行追加 → Write `/home/ubuntu/bochi-data/seen.jsonl`
</HARD-GATE>

## Gotchas (CRITICAL)

- gog CLI is NOT installed. Google data comes from cache/*.md (S3 sync from Mac).
  - cache が 2時間超の場合: 「最終同期: {google_synced_at}」を添えて表示
  - cache が存在しない場合: 「Macからの同期待ちゆ」と返答
...
- PreToolUse(Read): auto-pull from S3 when reading bochi-data (5s debounce)
  - E2E遅延: 典型3.5秒、最悪11秒
  - bochi-data 以外の Read は即 exit 0（レイテンシ影響なし）
...
1. `/home/ubuntu/bochi-data/` exists with `index.jsonl`
...
書き込みは /home/ubuntu/bochi-data/ にのみ行う。
