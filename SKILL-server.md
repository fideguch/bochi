...
All persistent data lives in `~/bochi-data/` (on Lightsail: `/home/ubuntu/bochi-data/`.`~/.claude/bochi-data` symlink exists but **書き込みパスに使用禁止** — Claude Code の .claude/ ハードコード保護が発動する):
...
| トリガーワード | Mode | 実行内容 |
|--------------|------|---------|
| 「おすすめ」「何か面白い？」「ぼちぼち話そう」「暇」 | Mode 3 | Read `references/casual-chat-spec.md` → **WebSearch で記事検索** → 3-5件提示 → seen.jsonl 追記 |
...
「おすすめ」「新聞」「bochi」等のトリガーワードを含む入力が来た場合は、
...
- ユーザーの文脈に合わせた短い返し
- 「雑談」と入力された場合: 「おすすめって言ってくれたら記事を提案するゆ ✨」と案内してから自然に会話を続ける
